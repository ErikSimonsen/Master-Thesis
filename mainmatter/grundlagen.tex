...
Sobald die Schedulingstrategie eines Schedulers erfordert, dass ein gerade rechnender Thread die Kontrolle einer \glsuseri{logische CPU}(*) an
...
Der Begriff Demultiplexing beschreibt dabei das Aufteilen eines, aus mehreren Signalen bestehenden, Signals
in seine ursprünglichen Komponenten.

Mit diesem Mechanismus lässt sich eine Menge an Ressourcen, wie Sockets oder Dateihandles, beobachten.
...
für die jeweilige Operation ist gibt der Demultiplexer eine Menge an Events zurück die anschließend in einer
...
Sobald eine I/O-Operation abgeschlossen ist und mindestens eine der beobachteten Ressourcen wieder verfügbar sind,
...
Während der Event-Handler ausgeführt wird, kann dieser weitere I/O-Operationen ausführen, durch die wiederrum
...
die Programmierung von hochperformanten Low-Level Netzwerk-Anwendungen, wie TCP- oder UDP-Socket Server simplifiziert, indem
...
Gegenüber Frameworks die ähnliche Funktionalität bieten, allerdings auf \verb|Blocking I/O| basieren, resultiere außerdem die Nutzung von Netty
in besserem Durchsatz, niedrigerer \Gls{latenz}(*) und geringerem Ressourcenverbrauch. \parencite{Netty}
