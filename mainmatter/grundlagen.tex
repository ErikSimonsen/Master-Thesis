...
vom Scheduler \& Dispatcher des Betriebssystems übernommen \cite{Tanenbaum2016}.
...
gewechselt werden muss, wird bei einem Threadwechsel lediglich der Inhalt der CPU-Register (inkl. Programmzähler) ersetzt\cite{Brosenne2021}\cite{Mosberger2002}.
...
können Programme ohne jeglichen Zustand realisiert werden\cite{Escoffier2017}.
...
\begin{figure}[h!]
    ...
    \caption{Exemplarische Abbildung einer Event Loop. \cite[Kapitel 1.7]{Ponge2020}}
\end{figure}
...
keine blockierenden, oder zeitintensiven Operationen beinhalten\cite{Ponge2020}.
...
\cite{Oracle2021} \footnote{Im Idealfall würde auf jedem CPU Kern ein \textit{worker thread} laufen,
...
\end{enumerate}\cite{JavaSpektrum2015}
...
sie in Form einer eigenen Java-Spezifikation namens \textit{Reactive Streams}-API implementiert und veröffentlicht haben.\cite{ReactiveStreams}
...
(\textit{message queues}) und, falls nötig, Anwenden von \textit{back pressure}, erlaubt.\cite{ReactiveSystems}
...
werden können.
Um in Java einzelne, asynchrone Prozesse zu implementieren, wird vom JDK die Future-API zur Verfügung gestellt.\cite{OracleFuture}
Für die Verarbeitung von asynchronen (unbegrenzten) Datenströmen gibt es die Flow-API.\cite{OracleFlow}
...
Operatoren und dem Grad der funktionalen Programmierung.\cite{ReactiveX, Mutiny}
...
\end{enumerate}\cite{Vert.x, Akka, ProjectReactor}
...
als nativ ausführbare Anwendung: dem \textit{native image} (native mode). Dafür nutzt Quarkus eine, von Oracle entwickelte, Technologie namens GraalVM.
Dabei handelt es sich um eine polyglotte, virtuelle Maschine und Laufzeitumgebung die auf dem OpenJDK basiert, und über
JVMCI \footnote{Java Virtual Machine Compiler Interface} den C2-Compiler der zugrundeliegenden HotSpot-JVM durch den Graal JIT-Compiler ersetzt.\cite{GraalVM}
...
im Cloud-Umfeld bewirkt. \cite{RedHatQuarkusInfografik}
...
alle Netzwerk I/O-Operationen verarbeitet. \cite{QuarkusReactiveGettingStarted, Quarkus}