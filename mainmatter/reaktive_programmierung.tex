...
Da es sich nicht um ein Framework handelt, gibt es keine vordefinierte Grundlage für Anwendungen und kann dadurch auch in bestehende Projekte
integriert werden.
...
Eine Implementierung des Reactor-pattern nutzt standardmäßig einen einzigen \verb|Event loop|-Thread, der in einer Schleife alle
...
Statt einer einzigen Event Loop hält jede Vertx-Instanz mehrere, abhängig von der Anzahl der verfügbaren CPU-Kerne|, \verb|Event loop|-Threads.
Diese Threads werden aus einem im vornherein erzeugten \verb|Event loop thread pool| entnommen.
\footnote{Die Größe des \textit{event loop thread pools} kann auch manuell überschrieben werden}
//TODO: vergleich zu worker thread pools aus thread per request ansatz ? -> konzept ähnlcih aber um ein vielfaches kleiner -> und nur ein thread pro kern keine threadwechsel
...
//TODO: Falls blockierender Code (normal in bestehnden Systsemen und Ökosystemen) wird deer Anwendungscode, ansttatt die Event Loop zu blockieren
// auf einen Worker-Thread (aus einem vornhereein erzeugen Worker Thred pool) dispatched
\subsubsection{Frameworks}
\label{subsubsec:frameworks}
//TODO: Komplette Anwendungsframeworks welche Gebrauch von den oben genannten Frameworks machen, aber auch noch viel mehr anbieten und viel Konfiguration
// wegabstrahieren, bspw.
\begin{itemize}
	\item Spring WebFlux
	\item Quarkus
\end{itemize}

Fullstack Frameworks SpringWebFlux nutzt Project Reactor under the Hood, Quarkus nutzt Vert.x under the hood
\paragraph{Quarkus}
